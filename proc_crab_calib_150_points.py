...
for name in tqdm(files_0531):
    ...
    """
    fName_plot =  './final_dataset/obs_plot/' + head['date'] + '_plot_'+ head['name'] + '.png'
    i = 0
    ...
    while np.max(test_flat_obser) >= 1800:
        ...
        path_pulse = './final_dataset/obs_plot/' + head['date'] + '_plot_'+ head['name'] + '_'+ str(i)  + '.png'
        plt.close()
        plt.title(
...
gp_crab.to_csv('crab_gp_kaz_10_2018-2019_calib_150_points.csv',  sep='\t', header=True, index=False)
